...
extern __m256i bishop_mask_v4[SQUARE_NB];
extern __m128i rook_mask_NS[SQUARE_NB];
...
template<> inline Bitboard attacks_bb<QUEEN>(Square s, Bitboard occupied)
{
  ...
  __m256i slide4 = _mm256_and_si256(occupied4, lmask);
  ...
  __m256i rslide = _mm256_srav_epi64(_mm256_set1_epi64x(0x8000000000000000),
...
template<> inline Bitboard attacks_bb<ROOK>(Square s, Bitboard occupied)
{
#if defined(__AVX512CD__) && defined(__AVX512VL__)
  ...
  const __m128i rmask = _mm256_castsi256_si128(queen_mask_v4[s][1]);
    ...
    // Left bits: set mask bits lower than occupied LS1B
  __m128i slide2 = _mm_and_si128(occupied2, lmask);
  ...
  slide2 = _mm_ternarylogic_epi64(slide2, _mm_add_epi64(slide2, _mm_set1_epi64x(-1)), lmask, 0x28);
    // Right bits: set mask bits higher than occupied MS1B
  __m128i rslide = _mm_srav_epi64(_mm_set1_epi64x(0x8000000000000000),
...
#else
    // South: flip vertical to simulate MS1B by LS1B
  ...
  const __m128i mask = rook_mask_NS[s];
    // North: set mask bits lower than occupied LS1B
...
template<> inline Bitboard attacks_bb<BISHOP>(Square s, Bitboard occupied)
{
