...
__m256i bishop_mask_v4[SQUARE_NB];
#endif
  ...
  for (Square sq = SQ_A1; sq <= SQ_H8; ++sq)
    for (int j = 0; j < 2; ++j) {
      for (int i = 0; i < 4; ++i) {
        vecAttacks[i] = 0;
        Square s = sq;
        while(safe_destination(s, vecDirections[j][i]))
            vecAttacks[i] |= (s += vecDirections[j][i]);
      }
      queen_mask_v4[sq][j] = _mm256_set_epi64x(vecAttacks[3], vecAttacks[2], vecAttacks[1], vecAttacks[0]);
    }

  // pseudo attacks for Rook (NORTH-SOUTH) and Bishop
  for (Square sq = SQ_A1; sq <= SQ_H8; ++sq) {
    rook_mask_NS[sq] = _mm_set_epi64x(
      _mm256_extract_epi64(queen_mask_v4[flip_rank(sq)][0], 1),	// SOUTH (vertically flipped)
      _mm256_extract_epi64(queen_mask_v4[sq][0], 1));	// NORTH
    bishop_mask_v4[sq] = _mm256_set_epi64x(
      _mm256_extract_epi64(queen_mask_v4[flip_rank(sq)][0], 2),	// SOUTH_EAST (vertically flipped)
      _mm256_extract_epi64(queen_mask_v4[sq][0], 3),	// NORTH_WEST
      _mm256_extract_epi64(queen_mask_v4[flip_rank(sq)][0], 3),	// SOUTH_WEST (vertically flipped)
      _mm256_extract_epi64(queen_mask_v4[sq][0], 2));	// NORTH_EAST
  }

  // sliding attacks for Rook EAST-WEST
  for (int occ = 0; occ < 128; occ += 2)	// inner 6 bits
    for (int sq = 0; sq < 8; ++sq) {
      uint8_t attack = 0;
      for (uint8_t s = (1 << sq) << 1; s; s <<= 1) {
	attack |= s;
        if (occ & s)
          break;
      }
